...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql://password@localhost/wallpapers'
db = SQLAlchemy(app)
...
def dbquery():
	...
	if(database=="wallpapers"):
		results = Wallpapers.query.filter_by(image=os.path.basename(query)).all()

	if(database=="floors"):
		results = Floors.query.filter_by(image=os.path.basename(query)).all()
